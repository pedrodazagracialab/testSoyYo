...
public class SwiftSpinner: UIView {
    ...
    public override init(frame: CGRect) {
        ...
        let viewBackGround = UIView(frame: CGRect(x: 0, y: 0, width: screenWidth, height: screenHeight))
        
        viewBackGround.backgroundColor = #colorLiteral(red: 0.4392156863, green: 0.003921568627, blue: 0.6235294118, alpha: 1)
        addSubview(viewBackGround)
        
        anim.animation = Animation.named("hello", bundle: Bundle.main)
        anim.translatesAutoresizingMaskIntoConstraints = false
        anim.contentMode = .scaleAspectFit
        anim.loopMode = .loop
        viewBackGround.addSubview(anim)
        anim.heightAnchor.constraint(equalToConstant: 200).isActive = true
        anim.widthAnchor.constraint(equalToConstant: 200).isActive = true
        anim.centerYAnchor.constraint(equalTo: centerYAnchor).isActive = true
        anim.centerXAnchor.constraint(equalTo: centerXAnchor).isActive = true
        anim.play()
